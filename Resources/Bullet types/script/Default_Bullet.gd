...
func shoot(cannonball_scene: PackedScene, player:Player, isometric = false, scale = 1.0):
	var cannonball_shot1 = cannonball_scene.instantiate()
	var cannonball_shot2 = cannonball_scene.instantiate()
	var upgraded_cannon_ball = apply_cannonball_upgrades(cannonball_shot1)
	var upgraded_cannon_ball2 = apply_cannonball_upgrades(cannonball_shot2)
